...
@Component({
  selector: 'app-qrstatic',
  templateUrl: './qrstatic.component.html',
  styleUrls: ['./qrstatic.component.css']
})
export class qrstaticComponent implements OnInit {
  costumers: any;
  ctmr: any;
  err:any;
  transactions:any;
  ...
  httpClient: any;
  imageToShow: any;
 // blobimage: { [k: string]: any; };
  blobimage:Blob;
  timesession : any;
  img1 : any;
  
  constructor(private service:GipService,private router :Router,private bnIdle: BnNgIdleService) {
   // this.blobimage= router.getCurrentNavigation().extras.state.qrstatic;

   if (typeof (router.getCurrentNavigation().extras.state) !== 'undefined'  ){
    this.createImageFromBlob(router.getCurrentNavigation().extras.state.qrstatic);
   }
   // this.createImageFromBlob(router.getCurrentNavigation().extras.state.qrstatic);
    //console.log(router.getCurrentNavigation().extras.state.qrstatic);
   }
