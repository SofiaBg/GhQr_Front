...
import { Registrationqrstatic } from '../services/Registrationqrstatic';
...
@Component({
  selector: 'app-new-qrstatic',
  templateUrl: './new-qrstatic.component.html',
  styleUrls: ['./new-qrstatic.component.css']
})
export class NewqrstaticComponent implements OnInit {
  user:any;
  banques: any;
  mode:number=0;
  errorMessage:string;
...
*/
  onRegister1(qrstatic){
   console.log(qrstatic);
...
*/
   this.gipService.register1(qrstatic).subscribe(
      data=>{
        console.log("start");
        this.qrstatic=data;
        this.mode= 1;
        this.show=true;
        console.log(" qrstatic returned "+this.qrstatic);
    ...
    /*    let route = this.router.config.find(r => r.path === '/qrstatic');
      ...
      let navigationExtras: NavigationExtras = {
        state: {
          qrstatic: this.qrstatic
        }
      };
      this.router.navigate(['/qrstatic'], navigationExtras);
  ...
  getImageFromService() {
    ...
    this.imageService.getImage(this.qrstatic).subscribe(data => {
...
getImage(imageUrl: string): Observable<Blob> {
  return this.httpClient.get(imageUrl, { responseType: 'blob' });
   ...
   autochargement(){
    ...
    this.router.navigateByUrl("/addqrstatic")
