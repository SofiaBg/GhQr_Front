...
export class BulkMerchantsValidationComponent implements OnInit {
  ...
  onsizechange() {
    ...
    this.showResult = this.bulkMerchants.slice(this.currentpage * this.pagesize, (this.currentpage + 1) * this.pagesize);
  }
  ...
  validateBulkMerchantCreation(csvRecord : CSVRecord ){
    ...
    this.gipService.validateBulkMerchantCreation(csvRecord).subscribe(data => {
  ...
  rejectBulkMerchantCreation(csvRecord : CSVRecord ){
    ...
    this.gipService.rejectBulkMerchantCreation(csvRecord).subscribe(data => {
      console.log('DATA ', data)
      if(data['respCode'] == "000"){
        this.success = true;
        this.error = false;
